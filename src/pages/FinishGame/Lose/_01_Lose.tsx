    ...
    <div className='flex h-full w-full flex-col justify-center gap-4'>
      ...
      <div className='flex flex-col gap-2'>
        ...
        <div className='self-center whitespace-nowrap text-center text-sm font-extrabold tracking-tight text-primary-green'>
          {isChallenger
            ? `You put Daniel ${getAnswer(challenger_answer)}`
            : `${shortenAddress(challenger_address)} put Daniel ${getAnswer(
                challenger_answer
              )}`}
        </div>
        <div className='self-center whitespace-nowrap text-center text-sm font-extrabold tracking-tight text-primary-green'>
          {!isChallenger
            ? `You guessed Daniel was ${getAnswer(opponent_answer)}`
            : `${shortenAddress(opponent_address)} guessed Daniel was ${getAnswer(
                opponent_answer
              )}`}
        </div>
      </div>
